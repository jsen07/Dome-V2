...
const ImageCropper = () => {
  ...
  const getCroppedImage = () => {
    ...
    image.onload = () => {
      ...
      ctx.drawImage(
        image,
        crop.x / scaleFactor, crop.y / scaleFactor, crop.width / scaleFactor, crop.height / scaleFactor,  
  ...
  const uploadCroppedImage = async () => {
    ...
    try {
      ...
      const storageRef = ref(storage, `background-images/${userId}-background.png`);
...
{crop && (
          <button onClick={getCroppedImage}> Confirm </button>
)}
      {croppedImageUrl && (
        <div>
