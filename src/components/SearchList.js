...
const SearchList = ({ results, key }) => {
  ...
  const createChat = async () => {
    ...
    try {
      ...
      const userNotificationKey = push(
        ref(db, `chatList/${user.uid}/notifications`)
      ).key;
      const recieverNotificationKey = push(
        ref(db, `chatList/${results.uid}/notifications`)
      ).key;
      ...
      if (!chatSnapshot.exists()) {
        ...
        await Promise.all([
          ...
          //set notification
          set(
            child(ref(db), `chatList/${user.uid}/notifications/${results.uid}`),
            {
              messages: {},
            }
          ),
          set(
            child(ref(db), `chatList/${results.uid}/notifications/${user.uid}`),
            {
              messages: {},
            }
          ),
        ...
        <div className="searched-user__box">
          ...
          <div className="searched-user__view">
            ...
            <div className="searched-view">
              <div
                onClick={() => navigate(`/profile?userId=${results.uid}`)}
                id="view-user"
              ></div>
            </div>
          </div>
