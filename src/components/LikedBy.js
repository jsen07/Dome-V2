...
import { getDatabase, ref, get, set, push,  child, serverTimestamp } from "firebase/database";
    ...
    try {
        ...
        for (const userId of postLikes) {
            ...
            if (snapshot.exists()) {
                ...
                const friendsSnapshot = await get(child(dbRef, `friendsList/${currentUser.uid}/${user.uid}`));
        ...
        {profileData.length > 0 ? (
             profileData.map((user) => (
                 ...
                 {hasSent ? (
                    ...
                    <Button className='send-message-button' variant="outlined" startIcon={<PersonAddIcon/>} onClick={()=> handleFriendRequest(user.uid)}>
                    Send friend request
                  </Button>
                  )}
                  </>
